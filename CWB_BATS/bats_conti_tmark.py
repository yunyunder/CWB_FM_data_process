#!/bin/python
# modify by cut_cwb_continu_tmark.py
# read BATS mseed and pzfile
...
dict_pz = {}
sta_all  = os.listdir('PZ_BATS')
#print(sta_all)

for sta in dict_sta_info:
    PZs = glob.glob('PZ_BATS/SAC_PZs_TW_%s*'%(sta))
    if (PZs == []):
        #print(sta)
        continue
    print(PZs[-1])
...
#for i in np.arange(1) :
    ...
    for sta in dict_pz :
        ...
        st = Stream()
        st_sta = Stream()
        mseedpath = ('/home/seismograms/TWseismograms/BATS/%s/%s')%(sta, starttime.strftime('%Y'))
        if not os.path.exists(mseedpath) : 
            continue
        if glob.glob('%s/*HH*.%s'%(mseedpath, starttime.strftime('%j'))) == [] :
            continue
        st = read('%s/*HH*.%s'%(mseedpath, starttime.strftime('%j')))
        print(st)
        print('')
        ...
        for tr in st_sta :
            ...
            for phase in ['p', 's']:
                ...
                name = ('%s/%s.%s.%s.SAC')%(event_dir, tr.stats.network, tr.stats.station, tr.stats.channel)
